...
const selectedChildrenList = ref<string[]>([]);
const scheduleList = ref<TScheduleSlot[]>([{} as TScheduleSlot]);

const currentGroup = computed(() => groupStore.getCurrentGroup);
...
const onSubmit = async (): Promise<void> => {
  ...
  try {
    ...
    const childrenListSelected = childrenList.value.filter((child: TChild) => {
      return selectedChildrenList.value.includes(child.id);
    });

    const formData: TGroup = {
      groupId: currentGroup.value.groupId,
      name: groupName.value.trim(),
      schedule: [...scheduleList.value],
      children: childrenListSelected,
      subscriptionPrice: subscriptionPrice.value,
    };

    await groupStore.UPDATE_GROUP(formData);
    await navigateTo("/");
  } finally {
...
const initForm = (group: TGroup) => {
  groupName.value = group.name;
  subscriptionPrice.value = group.subscriptionPrice;
  selectedChildrenList.value = group.children.map((child: TChild) => child.id);
  scheduleList.value = [...group.schedule];
};

const { pending } = await useAsyncData(
  `GET_GROUP_${groupId.value}`,
  async () => {
    const [group] = await Promise.all([
      groupStore.GET_GROUP_BY_ID(groupId.value),
      childrenStore.GET_CHILDREN_LIST(),
    ]);
    initForm(group);
    return true;
  },
